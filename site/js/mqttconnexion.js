...
function onMessageArrived(r_message){// quand un message arrive
	try {
		...
		if(dataJson.hasOwnProperty('distance')&&(connected_flag==1)){ // si dans le json la clé 'E_Lux' existe...

			temps = Number(dataJson.temps);
			distance = Number(dataJson.distance);
			detection = String(dataJson.detection);
			console.log("la distance : "+distance);
			gauge1.value = distance;// mets à jour la jauge éclairement
			$('#ECL1').html(distance);// rajoute la valeur de l'éclairement dans l'élément HTML d' ID ECL1 
			$('#DETECT').html(detection);
